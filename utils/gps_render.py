...
class GPSImageRender(Render):
    ...
    def render(self, ix, iy):
        mid = f"{ix}_{iy}"
        gps = cv2.imread(os.path.join(self.gps_root, f"{mid}_gps.{self.ext}"),
            # cv2.IMREAD_GRAYSCALE
            cv2.IMREAD_UNCHANGED
        )
        if gps is None:
            print("error in gps:", os.path.join(self.gps_root, f"{mid}_gps.{self.ext}"))
        # gps = cv2.resize(gps, (1024, 1024))
        # gps = np.expand_dims(gps, axis=2)
        return gps

    def render_npy(self, ix, iy):
        mid = f"{ix}_{iy}"
        gps = np.load(os.path.join(self.gps_root, f"{mid}_gps.{self.ext}")) #sahpe = (b,h,w,c)
        gps = np.squeeze(gps)
